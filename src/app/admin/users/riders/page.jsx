...
function Page() {
  ...
  if (usersData.isError) {
    return <div>Error: {usersData.error?.message || "Failed to load riders"}</div>;
  }
  ...
  return (
    <InnerDashboardLayout>
      <div className="mb-0 flex items-center justify-between">
        <div>
          <h2 className="font-bold text-3xl mt-1">Riders</h2>
          <p className="text-gray-500 text-base">Manage riders — add new, search, export or refresh.</p>
        </div>
      ...
      {/* Stats */}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6 mt-5 w-full mb-6">
              <StatCard title="Riders" value={apiData?.totalCount ?? demo.totalRiders} Icon={PeopleIcon} color="#111827" />
      ...
      <div className="bg-white p-6 border border-gray-300 rounded-xl mt-5">
        <div className="mb-5 flex items-center justify-between">
          <div>
            <h2 className="font-semibold text-2xl">Riders Directory</h2>
            <p className="text-gray-500 text-sm">Search, filter, create or manage riders.</p>
          </div>
        </div>
        ...
        <div className="mb-4 flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
          <Box className="flex-1" sx={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
            ...
            <TextField
              variant="outlined"
              placeholder="Search riders, name, id..."
              size="small"
              fullWidth
              value={searchInput}
          ...
          <Box sx={{ minWidth: 180 }}>
            <FormControl fullWidth variant="outlined" size="small" sx={{ "& .MuiOutlinedInput-root": { borderRadius: "7px" } }}>
              <InputLabel id="rider-status-filter">Status</InputLabel>
              <Select labelId="rider-status-filter" label="Status" value={status} onChange={(e) => setStatus(e.target.value)}>
